...
{
	FILE* universefp;
	universefp=fopen("234101040_universe.csv","w");
	for(int d=0;d<=9;d++){
		for(int u=1;u<=TRAIN_SIZE;u++){
			char filename[60];
			sprintf(filename,"234101040_dataset/English/txt/234101040_E_%d_%02d.txt",d,u);
...
void load_codebook(){
	FILE* fp;
	fp=fopen("234101040_codebook.csv","r");
	if(fp==NULL){
		printf("Error in Opening File!\n");
		return;
	}
...
void createcodebook()
{
	load_universe("234101040_universe.csv");
	LBG();
	store_codebook("234101040_codebook.csv",K);
	load_codebook();
}
...
void train_model(int digit, int utterance){
	...
	FILE *fp;
	char filenameA[40];
	_snprintf(filenameA,40,"234101040_lambda/A_%d_%d.txt",digit,utterance);
	...
	//Store B in file
	char filenameB[40];
	_snprintf(filenameB,40,"234101040_lambda/B_%d_%d.txt",digit,utterance);
...
void calculate_avg_model_param(int d){
	...
	for(int u=1;u<=25;u++){
		char filenameA[100];
		sprintf(filenameA,"234101040_lambda/A_%d_%02d.txt",d,u);
		fp=fopen(filenameA,"r");
		for (int i = 1; i <= N; i++)
		{
			for (int j = 1; j <= N; j++)
		...
		fclose(fp);
		char filenameB[100];
		sprintf(filenameB,"234101040_lambda/B_%d_%02d.txt",d,u);
		fp=fopen(filenameB,"r");
		for (int i = 1; i <= N; i++)
		{
			for (int j = 1; j <= M; j++)
...
void store_final_lambda(int digit){
	FILE *fp;
	char filenameA[40];
	sprintf(filenameA,"234101040_lambda/A_%d.txt",digit);
	fp=fopen(filenameA,"w");
	for (int i = 1; i <= N; i++)
	{
		for (int j = 1; j <= N; j++)
	...
	fclose(fp);
	char filenameB[40];
	_snprintf(filenameB,40,"234101040_lambda/B_%d.txt",digit);
	fp=fopen(filenameB,"w");
	for (int i = 1; i <= N; i++)
	{
		for (int j = 1; j <= M; j++)
...
void processTestFile(int d){
	...
	initialization();
	char filenameA[40];
	_snprintf(filenameA,40,"234101040_lambda/A_%d.txt",d);
	fp=fopen(filenameA,"r");
	if (fp == NULL){
		printf("Error\n");
	}
	...
	char filenameB[40];
	_snprintf(filenameB,40,"234101040_lambda/B_%d.txt",d);
	fp=fopen(filenameB,"r");
	for (int i = 1; i <= N; i++){
		for (int j = 1; j <= M; j++){
			fscanf(fp, "%Lf ", &B[i][j]);
